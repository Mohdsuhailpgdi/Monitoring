...
# - added FortiGate managed forti-switches status check (offline, online)
#
...
# Get just the authorised switch data
    ...
    %switches_names  = map { (my $temp = $_ ) =~ s/^${oid_swstatetableroot}${oid_swname}\.//; $temp => $switches_names{$_}  } keys %switches_names;
    %switches_status = map { (my $temp = $_ ) =~ s/^${oid_swstatetableroot}${oid_swstatus}\.//; $temp => $switches_status{$_} } keys %switches_status;

    if (defined($whitelist) and length($whitelist))
    {
      delete $switches_names{$_} for grep { $switches_names{$_} !~ $whitelist } keys %switches_names;
    }
    if (defined($blacklist) and length($blacklist))
    {
      delete $switches_names{$_} for grep { $switches_names{$_} =~ $blacklist } keys %switches_names;
    }
    my %switches = map {
      $_ => {
    ...
    if ($fswitchsdown > 0 and $mode >= 1) {
      $return_string_errors .= sprintf("DOWN[%s]", join(", ", @switches_down));
    }

  #Set Unitstate
  if ($mode >= 2 )  {
    if ($fswitchsdown == 1) { $return_state = "WARNING"; }
    if ($fswitchsdown >= 2) { $return_state = "CRITICAL"; }
  }
